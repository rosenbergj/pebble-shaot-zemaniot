...
Why the face looks and behaves the way it does: the constraints the Pebble
platform imposes, the rules behind each thing on screen, and the measurements
that settled the layout. Every decision here was reached by building the
alternative and looking at it, usually on the watch rather than the emulator.

For how to build, test, and release the face, see [DEVELOPING.md](DEVELOPING.md).

## Type sizing in the boxes
...
24pt line was centered on, instead of dropping it toward the bottom edge or
...
- **Cache values, not formatted text.** There is no event for a change to the
  ...
  draw time; formatting is cheap, and only the solar math has to stay out of
  the update proc.
...
over AppMessage; the watch never touches the network. Modeled on TimeStyle,
...
- **To force a location update, restart the phone-side JavaScript.** Switching
  ...
  the solar math and the weather follow within seconds. **Saving settings does
...
**Abbreviating won, and the reason is the neighbors.** 24 was worn first and
...
That budget forced the solar math onto the phone, so the face drifted wrong
